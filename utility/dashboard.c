...
static void render_adsb_rows(unsigned int columns, unsigned int max_rows,
    ...
    clear_line();
    if (columns >= 112u) {
        fputs("  ICAO    VOLO      QUOTA    SPD    TRK   V/RATE      LAT        LON       MSG   AGE\r\n", stdout);
    } else {
        fputs("  ICAO    VOLO      QUOTA    SPD    TRK      MSG   AGE\r\n", stdout);
    }
    for (unsigned int row = 0u; row < max_rows; row++) {
        ...
        clear_line();
        if (columns >= 112u) {
            printf("%c %06X  %-8.8s  ", stale, item->icao,
...
static void dashboard_render(int force) {
    ...
    tracked = equals_icase(g_dashboard.protocol, "ais")
                  ? cleanup_and_count_ais(now_wall)
                  : cleanup_and_count_adsb(now_wall);
